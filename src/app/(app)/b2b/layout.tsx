...
import { Home, Navigation, ListOrdered, User } from "lucide-react";
import { ROUTES } from "@/constants/app";

const NAV = [
  { id: "route",   label: "Ma Route", icon: Home,        href: ROUTES.b2bDashboard },
  { id: "track",   label: "Suivi",    icon: Navigation,  href: ROUTES.b2bTracking  },
  { id: "trips",   label: "Trajets",  icon: ListOrdered, href: ROUTES.b2bTrips     },
  { id: "profile", label: "Profil",   icon: User,        href: ROUTES.b2bProfile   },
];

/** Auth screen — nav hidden. */
const HIDE_NAV = ["/b2b", "/b2b/"];
...
export default function B2BLayout({ children }: { children: React.ReactNode }) {
  ...
  const activeId = (() => {
    if (pathname.startsWith("/b2b/dashboard") || pathname.startsWith("/b2b/change")) return "route";
    if (pathname.startsWith("/b2b/tracking")) return "track";
    if (pathname.startsWith("/b2b/trips"))    return "trips";
    if (pathname.startsWith("/b2b/profile"))  return "profile";
    return "";
  })();
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ background: active ? "rgba(229,57,53,0.10)" : "transparent" }}
                    >
                      <Icon size={22} strokeWidth={active ? 2.2 : 1.8} style={{ color: active ? "#e53935" : "#999999" }} />
                    </div>
                    <span className="text-[10px] font-extrabold tracking-wide transition-colors" style={{ color: active ? "#e53935" : "#aaaaaa" }}>
                      {label}
                    </span>
                  </button>
                );
